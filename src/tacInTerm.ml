...
let of_snippet snippet =
  ...
  let scaffold = Scaffold.wrap ~before:"let () = Runtime.Registry.register_output begin" ~after:"end" scaffold in
  (* Compile the code *)
...
let interp ?loc ~poly genv sigma tycon { env; snippet = { source_code; compiled_file } } =
  (* Run the code *)
  let () = Loader.load_file compiled_file in
...
let glob_ocaml_printer { env; snippet = { source_code } } =
  let open Pp in
  PrinterBasic begin fun _env _evd ->
